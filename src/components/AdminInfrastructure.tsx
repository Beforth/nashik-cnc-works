...
function sortRows(a: InfraRow, b: InfraRow) {
...
export default function AdminInfrastructure() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/admin/infrastructure', { credentials: 'include' });
      if (res.status === 401) {
  ...
  function updateRow(key: string, patch: Partial<InfraRow>) {
    setItems((prev) =>
      prev.map((r) => {
    ...
    try {
      ...
      if (row._isNew) {
        ...
        const patchRes = await fetch(
          `/api/admin/infrastructure?id=${encodeURIComponent(row.id)}`,
          {
            method: 'PATCH',
            credentials: 'include',
  ...
  async function saveRow(row: InfraRow) {
    const key = rowKey(row);
    setSavingId(key);
    setBusy(true);
    setMessage('Saving…');
    ...
    try {
      if (row._isNew) {
        if (!row.name.trim() || !row.specs.trim()) {
          setMessage('Name and specifications are required before saving a new card.');
          setSavingId(null);
          setBusy(false);
          return;
        }
        ...
        setMessage('Infrastructure saved — it is ordered first on the website.');
      } else {
  ...
  if (loading) {
    return <p className="p-8 text-center text-muted-grey">Loading infrastructure…</p>;
  }
