...
export default function AdminEnquiries() {
  ...
  async function remove(id: string, name: string) {
    const ok = await confirm({
      ...
      message: `Delete enquiry from “${name}”? This cannot be undone.`,
      confirmText: 'Delete',
    });
    if (!ok) return;
    setMessage(null);
  ...
  async function bulkRemove() {
    ...
    const ok = await confirm({
      ...
      message:
        'Permanently remove the selected enquiries from the database. This cannot be undone.\n\nOnly the rows you checked on this page are included (up to 100 per action).',
          ...
          <>
            <div className="mb-3 flex flex-col gap-2 rounded-xl border border-border-grey/80 bg-white px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-sm text-muted-grey">
                {selectedIds.size > 0 ? (
                  <>
                    <span className="font-black tabular-nums text-navy">{selectedIds.size}</span> selected on this page
                  </>
                ) : (
                  <>Use the checkboxes to select enquiries, then delete in one step (max 100).</>
                )}
              </p>
              <button
                type="button"
                    ...
                    <React.Fragment key={row.id}>
                      ...
                      {open ? (
                        <tr className="border-b border-border-grey bg-bg-cloud/50">
                          <td colSpan={9} className="p-0" id={`enquiry-panel-${row.id}`}>
                            <div
